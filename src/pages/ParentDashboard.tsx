...
const notifications = [
  { type: "alert", msg: "Ayesha scored below 50% in Physics quiz #5", time: "2 hours ago" },
  { type: "success", msg: "Ayesha completed Computer Science Module 8", time: "1 day ago" },
  { type: "info", msg: "New assessment scheduled for Mathematics", time: "2 days ago" },
  { type: "success", msg: "Ayesha improved her Mathematics score by 12%", time: "3 days ago" },
];

const ParentDashboard = () => {
  return (
    <DashboardLayout title="Parent Dashboard" navItems={navItems} userName="Mr. Javaid" userRole="parent">
      {/* Child summary */}
      <motion.div
      ...
      >
        <div className="flex items-center gap-4 mb-4">
          <div className="w-14 h-14 rounded-full bg-primary/10 flex items-center justify-center text-primary font-bold text-xl">A</div>
          <div>
            <h2 className="text-xl font-bold">Ayesha Javaid</h2>
              ...
              <div key={c.name} className="flex items-center gap-4">
                <div className="flex-1">
                  <div className="flex justify-between mb-1">
                    ...
                    <span className="text-xs font-semibold text-muted-foreground">{c.grade}</span>
                  </div>
                  <Progress value={c.score} className="h-2" />
                </div>
                <span className={`text-sm font-bold w-10 text-right ${c.score >= 70 ? "text-primary" : c.score >= 50 ? "text-accent" : "text-destructive"}`}>
                  {c.score}%
                </span>
              </div>
            ))}
          ...
          <div className="space-y-3">
            {notifications.map((n, i) => (
              <div key={i} className="flex items-start gap-3 p-3 rounded-lg bg-muted/50">
                {n.type === "alert" ? <AlertTriangle className="w-4 h-4 text-destructive mt-0.5 shrink-0" /> :
                 n.type === "success" ? <CheckCircle2 className="w-4 h-4 text-primary mt-0.5 shrink-0" /> :
