...
const TeacherDashboard = () => {
  return (
    <DashboardLayout title="Teacher Dashboard" navItems={navItems} userName="Mr. Ali Zaidi" userRole="teacher">
            ...
            {students.map((s) => (
              <div key={s.name} className="flex items-center justify-between p-3 rounded-lg bg-muted/50">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className={`text-sm font-semibold w-10 text-right ${getStatusColor(s.status)}`}>
                    {s.score}%
                  </span>
                </div>
              </div>
        ...
        <div className="space-y-6">
          <div className="bg-card rounded-xl border border-border p-6">
            <h2 className="text-lg font-semibold mb-4 flex items-center gap-2">
              <AlertTriangle className="w-5 h-5 text-accent" /> At-Risk Alerts
            </h2>
            <div className="space-y-3">
          ...
          <div className="bg-card rounded-xl border border-border p-6">
            <h2 className="text-lg font-semibold mb-4 flex items-center gap-2">
              <CheckCircle2 className="w-5 h-5 text-primary" /> Class Overview
            </h2>
            <div className="space-y-3">
              {[
                { cls: "BSIT-A (Mathematics)", avg: 74, students: 35 },
                { cls: "BSIT-B (Physics)", avg: 68, students: 30 },
                { cls: "BSIT-A (Computer Science)", avg: 82, students: 28 },
              ].map((c) => (
                <div key={c.cls} className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium">{c.cls}</p>
                    <p className="text-xs text-muted-foreground">{c.students} students</p>
                  </div>
                  <span className="text-sm font-semibold text-primary">{c.avg}%</span>
                </div>
              ))}
            </div>
          </div>
